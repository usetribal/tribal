...
4. Add feature flag in `crates/lineage-adapters/Cargo.toml` (e.g. `foo = []`)
5. Register in `all_adapters(workspace_root)` in `lib.rs` behind `#[cfg(feature = "...")]`
6. Add fixture under repo-root `tests/fixtures/<agent>-history/`
...
| Agent | Locations |
|-------|-----------|
| Cursor | `.cursor/projects/*/agent-transcripts/`, `.cursor/agent-transcripts/`, `~/.cursor/projects/<key>/agent-transcripts/` |
...
- Use existing fixtures as templates: `tests/fixtures/claude-code-history/`, `cursor-history/`, `codex-history/`
