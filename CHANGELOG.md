...
- Heredoc file writes are now captured as provenance: agent shell edits (`cat > f << EOF … EOF`) previously vanished into an opaque terminal-command artifact; the adapter now recognizes the heredoc redirect and emits a `FileEdit` with the body as post-image, feeding normal line-object materialization. The heredoc-into-a-flag case (`git commit -m "$(cat<<)"`, `--body`) is rejected. On the dev corpus this roughly doubled resolved artifacts (738→1548) and line objects (3002→6786)
