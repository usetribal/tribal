...
- `git lineage sync --server URL [--token TOKEN] [--remote origin]` — push redacted sessions to a Lineage platform server: assembles a `sync-batch-v0` (conversations, line objects, decomposed commit links, blob manifest), uploads referenced blobs via `PUT /v0/blobs/{sha256}`, POSTs the batch, and caches the server-issued repo id in local git config; token falls back to `LINEAGE_TOKEN`; implements `specs/sync-protocol-v0.md`
