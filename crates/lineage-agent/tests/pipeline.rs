...
fn filter_agent_respects_selection() {
    assert!(ImportPipeline::filter_agent(&[AgentKind::Cursor], AgentKind::Cursor));
    assert!(!ImportPipeline::filter_agent(&[AgentKind::Claude], AgentKind::Cursor));
...
fn import_all_combines_sources() {
    let fixture =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../tests/fixtures/cursor-history");
    let pipeline = ImportPipeline::default();
    let result = pipeline.import_all(&[(
