...
export const syncResponseSchema = z.object({ "metadata": z.record(z.string(), z.any()).optional(), "repo_id": z.string().describe("Server-resolved platform repo id; cache and send back as\n`repo.platform_repo_id`."), "results": z.array(z.object({ "id": z.string().describe("The object's id: ULID, `conversation_id:commit_sha` for links,\nsha256 hex for blobs."), "kind": z.enum(["conversation","turn","line_object","session_commit_link","blob"]), "reason": z.union([z.enum(["hash_mismatch","private","schema_version","too_large","invalid"]), z.null()]).optional(), "status": z.enum(["accepted","noop","rejected","pending"]) })), "schema_version": z.string() })
